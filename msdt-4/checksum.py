...
def calculate_checksum(row_numbers: List[int]) -> str:
    ...
    """
    logger.info('Начало вычисления md5 хеша.')
    row_numbers.sort()
    hash_sum = hashlib.md5(json.dumps(row_numbers).encode('utf-8')).hexdigest()
    logger.success(f'Получен хеш {hash_sum}.')
    return hash_sum
...
def serialize_result(variant: int, checksum: str) -> None:
    ...
    """
    logger.info('Начало сериализации результатов.')
    result = {
        "variant": variant,
        "checksum": checksum
    }
    ...
    logger.success('Сериализация успешно завершена.')
